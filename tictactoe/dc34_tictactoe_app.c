...
#define TAG "Dc34TicTacToe"

static const uint8_t DC34_TICTACTOE_AID[] = {0xF0, 0x43, 0x42, 0x54, 0x54, 0x54, 0x01};
...
typedef struct {
    ...
    uint16_t sw;
    bool have_board;
    Dc34TicTacToeState board_state;
} Dc34Result;
...
static const char* dc34_sw_to_text(uint16_t sw) {
    switch(sw) {
    case DC34_SW_SUCCESS:
        return "OK";
    case DC34_SW_NO_ACTIVE_GAME:
        return "No active game";
    case DC34_SW_GAME_OVER:
        return "Game already over";
    case DC34_SW_CELL_OCCUPIED:
        return "Cell occupied";
    case DC34_SW_INVALID_POSITION:
        return "Invalid position";
    case DC34_SW_CLA_NOT_SUPPORTED:
        return "CLA not supported";
    case DC34_SW_INS_NOT_SUPPORTED:
...
static NfcCommand dc34_poller_callback(NfcGenericEvent event, void* context) {
    ...
    /* 2. Auto-start a new game, human first. */
    payload_len = sizeof(payload);
    if(!dc34_transceive_apdu(
           poller,
           tx,
    ...
    result.sw = sw;
    if(sw == DC34_SW_SUCCESS &&
       dc34_parse_tictactoe_response(payload, payload_len, &result.board_state)) {
        result.have_board = true;
    }
    dc34_push_result(app, &result);
    if(sw != DC34_SW_SUCCESS) goto cleanup_stop;
    ...
    while(true) {
        ...
        uint8_t ins =
            (cmd.type == Dc34CmdNewGame) ? DC34_TICTACTOE_INS_NEW_GAME :
                                            DC34_TICTACTOE_INS_PLAY_MOVE;
...
static void dc34_board_draw_callback(Canvas* canvas, void* ctx) {
    ...
    if(model->connecting) {
        ...
        canvas_draw_str(canvas, 8, 24, "TicTacToe");
        canvas_set_font(canvas, FontSecondary);
    ...
    for(int row = 0; row < 3; row++) {
        for(int col = 0; col < 3; col++) {
            ...
            canvas_draw_frame(canvas, x, y, cell, cell);
            if(model->board[i] == 1) {
                canvas_draw_str(canvas, x + 6, y + 13, "X");
            } else if(model->board[i] == 2) {
                canvas_draw_str(canvas, x + 6, y + 13, "O");
            }
            if(i == model->cursor && model->status == DC34_TIC_TAC_TOE_STATUS_IN_PROGRESS &&
               !model->busy && !model->session_dead) {
                canvas_draw_frame(canvas, x + 2, y + 2, cell - 4, cell - 4);
            }
        }
    }

    int text_x = grid_x + 3 * cell + 6;
    int max_width = 128 - text_x - 2;
    canvas_draw_str(canvas, text_x, 12, "You: X");
    canvas_draw_str(canvas, text_x, 24, "Card: O");

    if(model->status_line[0] != '\0') {
        dc34_draw_wrapped(canvas, text_x, 36, 10, max_width, 3, model->status_line);
    }
}
...
static bool dc34_board_input_callback(InputEvent* event, void* ctx) {
    ...
    with_view_model(
        ...
        Dc34Model * model,
        {
            if(model->connecting || model->busy || model->session_dead) {
            ...
            } else if(model->status != DC34_TIC_TAC_TOE_STATUS_IN_PROGRESS) {
                ...
                switch(event->key) {
                case InputKeyUp:
                    if(model->cursor >= 3) model->cursor -= 3;
                    consumed = true;
                    break;
                case InputKeyDown:
                    if(model->cursor <= 5) model->cursor += 3;
                    consumed = true;
                    break;
                case InputKeyLeft:
                    if(model->cursor % 3 != 0) model->cursor -= 1;
                    consumed = true;
                    break;
                case InputKeyRight:
                    if(model->cursor % 3 != 2) model->cursor += 1;
                    consumed = true;
                    break;
                case InputKeyOk:
                    if(model->board[model->cursor] != 0) {
                        snprintf(
                            model->status_line,
                        ...
                        Dc34Cmd cmd;
                        cmd.type = Dc34CmdPlayMove;
                        cmd.p1 = (uint8_t)(model->cursor / 3);
                        cmd.p2 = (uint8_t)(model->cursor % 3);
                        if(furi_message_queue_put(app->cmd_queue, &cmd, 0) == FuriStatusOk) {
                            model->busy = true;
                            snprintf(
                                model->status_line, sizeof(model->status_line), "Sending...");
                        }
                    }
                    consumed = true;
                    break;
    ...
    while(furi_message_queue_get(app->result_queue, &result, 0) == FuriStatusOk) {
        with_view_model(
            ...
            Dc34Model * model,
            {
                ...
                if(!result.comm_ok) {
                ...
                } else if(result.sw != DC34_SW_SUCCESS) {
                    snprintf(
                        model->status_line,
                        sizeof(model->status_line),
                        "%s",
                    ...
                    if(result.have_board) {
                        memcpy(model->board, result.board_state.board, 9);
                        model->status = result.board_state.status;
                    }
                    switch(model->status) {
                    case DC34_TIC_TAC_TOE_STATUS_X_WINS:
                        snprintf(
                            model->status_line,
                            sizeof(model->status_line),
                            "You win! Ok=rematch");
                        break;
                    case DC34_TIC_TAC_TOE_STATUS_O_WINS:
                        snprintf(
                            model->status_line,
                            sizeof(model->status_line),
                            "Card wins. Ok=rematch");
                        break;
                    case DC34_TIC_TAC_TOE_STATUS_DRAW:
                        snprintf(
                            model->status_line, sizeof(model->status_line), "Draw! Ok=rematch");
                        break;
                    default:
                        snprintf(
                            model->status_line, sizeof(model->status_line), "Your move");
                        break;
                    }
                }
            },
...
int32_t dc34_tictactoe_app(void* p) {
    UNUSED(p);
